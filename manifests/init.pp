...
# Other Variables
  ...
  if $strictcrlpolicy { validate_re($strictcrlpolicy,'(yes|no)$',"${strictcrlpolicy} is not supported for strictcrlpolicy") }
  if $force_busy { validate_re($force_busy,'(yes|no)$',"${force_busy} is not supported for force_busy") }
