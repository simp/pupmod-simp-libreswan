# == Class: ipsec
#
# Module 'ipsec' installs and configures libreswan for use with IPSEC
...
#
# ---
# ipsec is designed to install and configure the ipsec service from libreswan.
...
# [*package_name*]
#   Type: String
#   Default:  +$::ipsec::params::package_name+
...
#   Type: Boolean
#   Default: +false+
#   If true, manage selinux to permit ipsec.
#
# [*enable_tcpwrappers*]
#   Type: Boolean
#   Default: +false+
...
class ipsec (
  ...
  $simp_logging = defined('$::simp_logging') ? { true => getvar('::simp_logging'), default => hiera('simp_logging',false) },
...
# Other Variables
  ...
  if $strictcrlpolicy != 'undef' { validate_re($strictcrlpolicy,'(yes|no)$',"${strictcrlpolicy} is not supported for strictcrlpolicy") }
  if $force_busy != 'undef' { validate_re($force_busy,'(yes|no)$',"${force_busy} is not supported for force_busy") }
  if $hidetos != 'undef' { validate_re($hidetos,'(yes|no)$',"${hidetos} is not supported for hidetos") }
  if $plutofork != 'undef' { validate_re($plutofork,'(yes|no)$',"${plutofork} is not supported for plutofork") }
  if $uniqueids != 'undef' { validate_re($uniqueids,'(yes|no)$',"${uniqueids} is not supported for uniqueids") }
  if $retransmits != 'undef' { validate_re($retransmits,'(yes|no)$',"${retransmits} is not supported for retransmits") }
  if $plutorestartoncrash != 'undef' { validate_re($plutorestartoncrash,'(yes|no)$',"${plutorestartoncrash} is not supported for plutorestartoncrash") }
  if $fragicmp != 'undef' { validate_re($fragicmp,'(yes|no)$',"${fragicmp} is not supported for fragicmp") }
  if $listen != 'undef' { validate_ipv4_address($listen) }
  if $perpeerlog != 'undef' { validate_re($perpeerlog,'(yes|no)$',"${perpeerlog} is not supported for perpeerlog") }
  if $nhelpers != 'undef' { validate_integer($nhelpers)}
  if $overridemtu != 'undef' { validate_integer($overridemtu)}
  if $keep_alive != 'undef' { validate_integer($keep_alive)}
  if $crlcheckinterval != 'undef' { validate_integer($crlcheckinterval)}
  if $myvendorid != 'undef' { validate_string($myvendorid)}
  if $statsbin != 'undef' { validate_string($statsbin)}
  if $syslog != 'undef' { validate_string($syslog)}
  if $plutostderrlog != 'undef' { validate_absolute_path ($plutostderrlog) }
  if $ipsecdir != 'undef' { validate_absolute_path($ipsecdir) }
  ...
  $token = $::ipsec::params::token

  include '::ipsec::install'
  ...
  include '::ipsec::config::pki'
  Class[ '::ipsec::config'  ] ->
  Class[ '::ipsec::config::pki']

  include '::ipsec::nsspki'
  Class[ '::ipsec::config::pki'  ] ~>
  Class[ '::ipsec::nsspki'  ]


  if $simp_firewall {
    include '::ipsec::config::firewall'
    Class[ '::ipsec::config::firewall' ] ~>
    Class[ '::ipsec::service'  ]
  }

  if $simp_logging {
    include '::ipsec::config::logging'
    Class[ '::ipsec::config::logging' ] ~>
    Class[ '::ipsec::service' ]
  }
