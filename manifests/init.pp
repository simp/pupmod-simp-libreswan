...
#   Only consumed when `firewall => true`. Defaults to the
#   `simp_options::trusted_nets` value on SIMP sites — this is a data
#   lookup, not a feature toggle, so it declares nothing by itself.
#
# @param firewall
...
#   consumed when `pki` is enabled. Defaults to the `simp_options::fips`
#   value on SIMP sites — this is a data lookup, not a feature toggle, so
#   it declares nothing by itself.
#
# @param pki
...
#   set this directly.
#
# @param manage_file_permissions
#   When `true`, restore the pre-5.0.0 ownership/permission management:
#   `/etc/ipsec.conf` (root, `0400`), `${ipsecdir}` (root, `0700`), and
#   `${dumpdir}` (root, `0700`, only when `dumpdir` is set). When `false`
#   (the default), the package-provided ownership and modes are left alone.
#
# @param nssdb_password
#   Password used to protect the NSS database. When `undef` (the default), a
#   password is generated and persisted with `simplib::passgen` — but only
#   when PKI management is enabled (`pki`), so a bare `include` creates no
#   server-side state.
#
# @param ikeport
...
#   Field names to remove from `/etc/ipsec.conf` if present. Use the names
#   as they appear in the file (hyphenated, e.g. `virtual-private`,
#   `ddos-ike-treshold`), not the class parameter names.
#
...
class libreswan (
  ...
  Boolean                                            $manage_file_permissions    = false,
  Optional[String[1]]                                $nssdb_password             = undef,
  ...
  # Whether the service resource is declared at all; also consulted by
  # libreswan::connection before wiring its notify edge
  $manage_service = $service_ensure =~ NotUndef or $service_enable =~ NotUndef
