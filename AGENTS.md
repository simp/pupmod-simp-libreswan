...
- **`libreswan::config` (private, `config.pp`)** — does NOT render
  ...
  **non-`undef`** entry, editing `/etc/ipsec.conf` in place with `file_line`.
  ...
  a top-level `include` with a syntax error. When any setting is managed, a
  `file_line` **guarantees the header exists** (appending one at EOF on a
  hand-edited file that lacks it — libreswan merges the extra section) and
  every managed setting `require`s it.
  When `$manage_file_permissions` is `true`, the pre-5.0.0 ownership/mode
  enforcement is restored (`/etc/ipsec.conf` root/0400, `$ipsecdir` root/0700,
  `$dumpdir` root/0700 when set).
...
- **The `simp_options::*` feature toggles are gone from `init.pp`.** Hiera
  keys like `simp_options::firewall`, `simp_options::pki`,
  `simp_options::haveged` no longer turn features on — set
  `libreswan::firewall`, `libreswan::pki`, `libreswan::haveged`, etc.
  directly (or use the `simp:defaults` profile). The *data* lookups stay:
  `simp_options::trusted_nets` and `simp_options::fips` (`init.pp` — they
  only shape opted-in behavior and declare nothing by themselves; the
  `simp:defaults` profile cannot carry site-specific values like these),
  `simp_options::pki::source` (`config/pki.pp`), `simp_options::fips`
  (`nss/init_db.pp`), and the module-local `iptables::use_firewalld`
  (`config/firewall.pp`).
