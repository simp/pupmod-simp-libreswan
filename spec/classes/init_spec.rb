    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'with a single setting overridden' do
          ...
          # The `config setup` anchor is guaranteed whenever settings are
          # managed, and every managed setting is applied after it.
          it {
            is_expected.to contain_file_line('libreswan /etc/ipsec.conf config setup header')
              .with(path: '/etc/ipsec.conf', line: 'config setup')
          }
          it {
            is_expected.to contain_libreswan__config__setting('plutodebug')
              .that_requires('File_line[libreswan /etc/ipsec.conf config setup header]')
          }
        end

        context 'with purge_settings only' do
          let(:params) { { purge_settings: ['plutodebug'] } }

          # Purging existing lines never inserts, so no anchor is needed.
          it { is_expected.to compile.with_all_deps }
          it { is_expected.not_to contain_file_line('libreswan /etc/ipsec.conf config setup header') }
        end

        context 'with manage_file_permissions => true' do
          let(:params) { { manage_file_permissions: true, dumpdir: '/var/run/pluto' } }

          it { is_expected.to compile.with_all_deps }
          it {
            is_expected.to contain_file('/etc/ipsec.conf')
              .with(ensure: 'file', owner: 'root', mode: '0400')
          }
          it {
            is_expected.to contain_file('/etc/ipsec.d')
              .with(ensure: 'directory', owner: 'root', mode: '0700')
          }
          it {
            is_expected.to contain_file('/var/run/pluto')
              .with(ensure: 'directory', owner: 'root', mode: '0700')
          }
        end

        context 'with manage_file_permissions => true and no dumpdir' do
          let(:params) { { manage_file_permissions: true } }

          it { is_expected.to compile.with_all_deps }
          it { is_expected.to contain_file('/etc/ipsec.conf').with_mode('0400') }
          it { is_expected.not_to contain_file('/var/run/pluto') }
        end
        ...
        # trusted_nets and fips are data lookups, not feature toggles: they
        # must follow simp_options::* like they did before 5.0.0, because the
        # simp:defaults profile cannot carry site-specific values.
        context 'with firewall => true and simp_options::trusted_nets in Hiera' do
          let(:hieradata) { 'simp_options_lookups' }
          let(:params) { { firewall: true } }

          it { is_expected.to compile.with_all_deps }
          it {
            is_expected.to contain_simp_firewalld__rule('ipsec_allow')
              .with_trusted_nets(['10.1.2.0/24'])
          }
        end

        context "with pki => 'simp' and simp_options::fips true in Hiera" do
          let(:hieradata) { 'simp_options_lookups' }
